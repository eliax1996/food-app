...
- Milestone-time cache measurement covered 64 representative one-page five-hit queries at 65,841 bytes; 2,048 projected to 2,106,912 bytes (about 2.01 MiB). Current fixture measurement is retained in `../docs/open-food-facts-search-assessment.md`; count cap governs typical data and 32 MiB guards long/multipage outliers.
- Milestone-time focused tests were client 11, cache 6, service 15, coordinator 8, with 85 hostless pass / 2 opt-in skips. Later final whole-tree hostless validation executed 243 tests (241 pass / 2 skips). Timed-out partial search responses remain rejected rather than cached as terminal.
...
- 2026-08-08: An ephemeral weight-navigation reviewer transcript (not retained) compared Apple Health, Happy Scale, Withings, Weight Diary Lite, and Weigh In; durable official URLs remain in `../docs/tracking-navigation-assessment.md`. Findings separated recording, chronological history, and analytics. Monitor Your Weight evidence retained with legacy screenshot caveat.
- 2026-08-08: A broad nutrition-navigation review timed out empty and contributed no evidence. A bounded transcript (not retained) then compared MacroFactor, Foodnoms, MyFitnessPal, and Cronometer against durable URLs in `../docs/tracking-navigation-assessment.md`. Initial nutrition evidence favors nested weight and remains relevant to Today/Progress; it does not override explicit discoverability feedback.
- 2026-08-08: An ephemeral nutrition-history transcript (not retained) found date-first meal diaries and separate weight histories; durable source URLs and conclusions remain in `../docs/tracking-navigation-assessment.md`.
- 2026-08-08: An ephemeral weight-history transcript (not retained) supported native newest-first list, row edit/delete, date handling, raw-entry preservation, and accessibility patterns; durable source URLs remain in `../docs/tracking-navigation-assessment.md`.
...
7. Review rows edit query and consumed g/ml amount, change selected product through candidate/saved/custom recovery, and show the selected record’s serving and nutrition basis. Arbitrary row-level serving/nutrition override is rejected: normalized amount already scales the verified record, while Custom Food owns explicit nutrition editing without silently altering a saved/Open Food Facts product. Every row remains marked for review until resolved.
