...
User-requested calorie-goal-derived theoretical gram ranges for carbohydrate, protein, and fat plus the energy-scaled Fiber reference are queued as `NUTRITION-GOALS-001` inside REFINE-001. That Plan feature will compare reference versus real measured intake without presenting one composition as universally ideal.
