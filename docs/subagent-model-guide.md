...
- Validation: parent installed iOS 17.5, normalized coordinator fixtures to production `AppModelSchema`, disabled coordinator autosave, strengthened selectors/diagnostics, and ran 377 app-hosted plus 55/55 isolated optimized UI journeys. Signed archive and pure Release iOS 17 bootstrap passed. Three final identical Terra `high` committed-tree reviewers returned 3/3 SOURCE APPROVE; all retained only external distribution/CI/device blockers.
