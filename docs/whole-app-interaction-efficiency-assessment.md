...
| **Meal detail — empty** | Log food to selected meal; Today → meal row | Flat meal sections → row **1**, Log food **1** | Floor: empty names meal and gives recovery; target: two actions to editor | Implemented native empty state; no invented meal. |
